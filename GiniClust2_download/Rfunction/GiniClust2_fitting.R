#Determine high Gini genes

#########################################################
# generate statistics table and LOESS curve fitting 
#########################################################
...
#ExprM.RawCounts.filter<-as.matrix(ExprM.RawCounts.filter)
...
# .6 save results
save(Genelist.top_pvalue, Genelist.HighNormGini, ExprM.Stat2, file=paste("results/", exprimentID,"_StatisticsTable_afterLOESS.RData",sep=""))
...
if(dim(GiniIndex)[2] > 2){
  ...
  write.csv(Gini_related_table_qPCR, file="results/Gini_related_table_qPCR.csv")
  
  pdf(paste("figures/", exprimentID, "_smoothScatter_normGini_genes.pdf", sep=""), width=6, height=6, useDingbats = FALSE)
  ...
  write.csv(Gini_related_table_RNAseq, file="results/Gini_related_table_RNA-seq.csv")
  
  pdf(paste("figures/", exprimentID, "_smoothScatter_pvalue_gene.pdf", sep=""), width=6, height=6, useDingbats = FALSE)
